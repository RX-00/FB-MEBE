...
def randomize_rigid_body_com_pos(
        env: ManagerBasedEnv,
        env_ids: torch.Tensor | None,
        asset_cfg: SceneEntityCfg,
        com_pos_distribution_params: dict[str, tuple[float, float]],
):
    ...
    coms_pos = coms[env_ids[:, None], body_ids, :3].clone()

    coms_pos[:, :, 0] += math_utils.sample_uniform(*com_pos_distribution_params["x"], coms_pos[:, :, 0].shape,
                                                   device=coms_pos[:, :, 0].device)
    coms_pos[:, :, 1] += math_utils.sample_uniform(*com_pos_distribution_params["y"], coms_pos[:, :, 1].shape,
                                                   device=coms_pos[:, :, 1].device)
    coms_pos[:, :, 2] += math_utils.sample_uniform(*com_pos_distribution_params["z"], coms_pos[:, :, 2].shape,
                                                   device=coms_pos[:, :, 2].device)

    coms[env_ids[:, None], body_ids, :3] = coms_pos
