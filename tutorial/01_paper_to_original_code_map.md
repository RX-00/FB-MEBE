...
|---|---|---|---|---|
| Reward-free MDP and online data collection | `ch03-background.tex`, Background | `scripts/reinforcement_learning/fb_mod/pretrain.py:214` | The training loop collects transitions from the vectorized Go2 environment while learning. | High |
| Successor measure factorization `F(s,a,z)^T B(s')` | `ch03-background.tex`, Eq. `fb_factorization` | `scripts/reinforcement_learning/fb_mod/agent_meta/fb/agent.py:254` | `update_fb` forms `Ms = F @ B.T` for contrastive TD learning. | High |
| FB temporal-difference contrastive loss | `ch03-background.tex`, Eq. `fb_loss` | `scripts/reinforcement_learning/fb_mod/agent_meta/fb/agent.py:254` | Uses target F/B networks, diagonal attraction, off-diagonal penalty, and target bootstrapping. | High |
| Backward-map orthonormality regularization | `Supplementary_Materials.tex`, FB details | `scripts/reinforcement_learning/fb_mod/agent_meta/fb/agent.py:299` | Adds covariance regularization on `B` to prevent representation collapse. | High |
| Actor objective `-F(s,a,z)^T z` | `ch03-background.tex`, Eq. `actor_loss` | `scripts/reinforcement_learning/fb_mod/agent_meta/fb/agent.py:362` | Actor samples actions and maximizes the FB Q estimate. | High |
| Regularized actor objective with `Q_reg` | `ch05-method.tex`, Eq. `actor_loss_with_reg`; `Supplementary_Materials.tex`, Regularized Exploration | `scripts/reinforcement_learning/fb_mod/agent_meta/fb/agent.py:391` and `agent.py:362` | Optional critic learns regularization reward and contributes to actor loss. | High |
| Reward inference `z_r = E[B(s) r(s)]` | `ch03-background.tex`, reward inference paragraph | `scripts/reinforcement_learning/fb_mod/agent_meta/fb/model.py:133`; `loader/fb_net_loader.py:70` | Projects command rewards over replay-buffer goals into a latent command for play/eval. | High |
...
| Inverse-density sampling `q(s)^(-beta)` | `ch05-method.tex`, Eq. `sample_inv` and `practical_fb_mebe` | `scripts/reinforcement_learning/fb_mod/density_estimator/agent_normalizing_flow.py:79` | Samples replay-buffer goals using inverse estimated density. | High |
...
| 80 percent MEBE / 20 percent random latent mix | `ch06-experiments.tex`, Baselines paragraph; `Supplementary_Materials.tex`, hyperparameters | `scripts/reinforcement_learning/fb_mod/configs/Isaaclab_pretrain_config_go2.yaml`; `agent.py:140` | `train_goal_ratio: 0.8` chooses inverse-density goals for most training latents. | High |
| Go2 robot with 12-D action space | `ch06-experiments.tex`, Environment paragraph | `source/isaaclab_tasks/isaaclab_tasks/direct/go2/env_default_abs/go2_cfg_base.py:25` | Sets action dimension to 12. | High |
| 200 Hz sim, 50 Hz control | `ch06-experiments.tex`, Environment paragraph | `source/isaaclab_tasks/isaaclab_tasks/direct/go2/env_default_abs/go2_cfg_base.py:23` and `go2_cfg_base.py:35` | `sim.dt = 1/200`, `decimation = 4`, so policy/control runs at 50 Hz. | High |
